...
export class ProberStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    proberFunction.addToRolePolicy(new iam.PolicyStatement({
      actions: ['compute-optimizer:GetEnrollmentStatus', 'organizations:DescribeAccount', 'organizations:DescribeOrganization', 'iam:ListUsers', 'budgets:ViewBudget', 'ce:GetAnomalyMonitors'],
      resources: ['*'],
      effect: iam.Effect.ALLOW
      })
    );
    ...
    for (let probeName of ['billing-invoice-by-email-enabled', 'billing-compute-optimizer-enabled', 'billing-iam-access-enabled', 'billing-tax-inheritance-enabled', 'billing-budget-created', 'billing-cost-anomaly-detector-created', 'security-account-is-organizations-management-account', 'security-account-has-no-iam-users']) {
